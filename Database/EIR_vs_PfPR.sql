...
select replicateid,
  filename,
  cast(substr(filename, 4, position('-' in substr(filename, 4)) - 1) as float) population,
...
where dayselapsed > 7305 and dayselapsed < 7701
  and replicateid in (select id from v_replicates where filename ~'^bf-[0-9]*-[0-9.]*.yml')
group by replicateid, filename
order by population, beta
