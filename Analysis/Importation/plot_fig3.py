...
def plot(dates, data):
    ...
    for year in [2033, 2034, 2035]:
        left.axvspan(datetime.datetime(year, 6, 1, 0, 0), datetime.datetime(year, 11, 1, 0, 0), alpha=0.5, color='#CCCCCC')
    ...
    left.legend(left_lines + right_lines, left_labels + right_labels, frameon=False)
    
    # Save the plot
    fig.savefig('plots/manuscript/MS BFA, Fig. 3.png', dpi=150)
    fig.savefig('plots/manuscript/MS BFA, Fig. 3.tif', dpi=300, format='tiff', pil_kwargs={"compression": "tiff_lzw"})
...
def add_plot(axis, dates, values, color, label, style='-'):
    ...
    axis.plot(dates, median, linewidth=2.5, linestyle=style, color=color, label=label)
